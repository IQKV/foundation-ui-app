 ...
 *                          In SINGLE_TENANT mode this is always true (platform tenant
 *                          is the user's only workspace). Organization UI is gated
 *                          separately via `isMultiTenantMode`.
 *
...
import { resolvePersonalWorkspaceFlag } from "@/shared/lib/rollout";
...
export const useSessionStore = create<SessionState>((set) => ({
  ...
  setTokens: (accessToken, refreshToken, tenantKey, isPersonalWorkspace) => {
    const resolvedPersonalWorkspace = resolvePersonalWorkspaceFlag(isPersonalWorkspace);
    saveToStorage(REFRESH_TOKEN_KEY, refreshToken);
    saveToStorage(TENANT_KEY_KEY, tenantKey);
    saveToStorage(IS_PERSONAL_WORKSPACE_KEY, String(resolvedPersonalWorkspace));
    set({ accessToken, refreshToken, tenantKey, isPersonalWorkspace: resolvedPersonalWorkspace });
  },
